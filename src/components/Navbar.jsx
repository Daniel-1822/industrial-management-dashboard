...
const Navbar = ({ logo, links, activeColor = '#FFC500' }) => {
    ...
    return (
        <>
            {/* Botón Hamburguesa Estilo Industrial - Solo visible en móvil */}
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="lg:hidden fixed top-4 left-4 z-[60] bg-industrial-dark p-3 border border-white/10 shadow-xl"
            >
                <div className="w-5 h-4 flex flex-col justify-between">
                    <div className={`h-0.5 bg-[#FFC500] transition-all ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
                    <div className={`h-0.5 bg-[#FFC500] transition-all ${isOpen ? 'opacity-0' : ''}`} />
                    <div className={`h-0.5 bg-[#FFC500] transition-all ${isOpen ? '-rotate-45 -translate-y-2' : ''}`} />
                </div>
            </button>
            ...
            <nav className={`
                fixed inset-y-0 left-0 z-50 w-64 bg-industrial-dark text-white h-screen
                transform transition-transform duration-300 ease-in-out
                ${isOpen ? 'translate-x-0' : '-translate-x-full'}
                lg:relative lg:translate-x-0 lg:flex-shrink-0
                flex flex-col pt-8 pb-8 overflow-hidden border-r border-white/5
            `}>
